<?php
// source: /var/www/nette/app/config/config.neon 
// source: /var/www/nette/app/config/config.local.neon 

class Container_a2d6a3effc extends Nette\DI\Container
{
	protected $meta = array(
		'types' => array(
			'Nette\Object' => array(
				array(
					...
					'session.session',
					'25_App_Model_Book',
					'26_App_Model_Category',
					'27_App_Model_Product',
					'28_App_Model_Search',
					'application.1',
					'application.2',
					'application.3',
			...
			'App\Model\Book' => array(1 => array('25_App_Model_Book')),
			'App\Model\Category' => array(1 => array('26_App_Model_Category')),
			'App\Model\Product' => array(1 => array('27_App_Model_Product')),
			'App\Model\Search' => array(1 => array('28_App_Model_Search')),
			...
			'App\Presenters\ProductPresenter' => array(array('application.3')),
			'App\Presenters\Error4xxPresenter' => array(array('application.4')),
			'App\Presenters\BookPresenter' => array(array('application.5')),
			'App\Presenters\ErrorPresenter' => array(array('application.6')),
			'App\Presenters\CategoryPresenter' => array(array('application.7')),
		...
		'services' => array(
			...
			'27_App_Model_Product' => 'App\Model\Product',
			'28_App_Model_Search' => 'App\Model\Search',
			'application.1' => 'App\Presenters\HomepagePresenter',
			'application.2' => 'App\Presenters\SearchPresenter',
			'application.3' => 'App\Presenters\ProductPresenter',
			'application.4' => 'App\Presenters\Error4xxPresenter',
			'application.5' => 'App\Presenters\BookPresenter',
			'application.6' => 'App\Presenters\ErrorPresenter',
			'application.7' => 'App\Presenters\CategoryPresenter',
	 ...
	 */
	public function createService__25_App_Model_Book()
	{
		$service = new App\Model\Book($this->getService('database.default.context'));
		return $service;
	}
	 ...
	 */
	public function createService__26_App_Model_Category()
	{
		$service = new App\Model\Category($this->getService('database.default.context'));
		return $service;
	}
	 ...
	 */
	public function createService__27_App_Model_Product()
	{
		$service = new App\Model\Product($this->getService('database.default.context'));
		return $service;
	}


	/**
	 * @return App\Model\Search
	 */
	public function createService__28_App_Model_Search()
	{
		$service = new App\Model\Search($this->getService('database.default.context'));
		return $service;
	}
	...
	{
		$service = new App\Presenters\SearchPresenter($this->getService('26_App_Model_Category'), $this->getService('25_App_Model_Book'),
			$this->getService('28_App_Model_Search'));
		$service->injectPrimary($this, $this->getService('application.presenterFactory'), $this->getService('routing.router'),
			$this->getService('http.request'), $this->getService('http.response'), $this->getService('session.session'),
			$this->getService('security.user'), $this->getService('latte.templateFactory'));
		$service->invalidLinkMode = 5;
	...
	{
		$service = new App\Presenters\ProductPresenter($this->getService('27_App_Model_Product'));
		$service->injectPrimary($this, $this->getService('application.presenterFactory'), $this->getService('routing.router'),
			$this->getService('http.request'), $this->getService('http.response'), $this->getService('session.session'),
			$this->getService('security.user'), $this->getService('latte.templateFactory'));
		$service->invalidLinkMode = 5;
	...
	{
		$service = new Nette\Application\PresenterFactory(new Nette\Bridges\ApplicationDI\PresenterFactoryCallback($this, 5, '/var/www/nette/app/../temp/cache/Nette%5CBridges%5CApplicationDI%5CApplicationExtension'));
	...
	{
		$service = new Nette\Caching\Storages\FileJournal('/var/www/nette/app/../temp');
		return $service;
	}
	...
	{
		$service = new Nette\Caching\Storages\FileStorage('/var/www/nette/app/../temp/cache', $this->getService('cache.journal'));
		return $service;
	}
	...
	{
		$service = new Nette\Database\Connection('mysql:host=127.0.0.1;dbname=nette', 'root', 'asdf', array('lazy' => TRUE));
	...
	{
		return new Container_a2d6a3effc_Nette_Bridges_ApplicationLatte_ILatteFactoryImpl_latte_latteFactory($this);
	}
...
final class Container_a2d6a3effc_Nette_Bridges_ApplicationLatte_ILatteFactoryImpl_latte_latteFactory implements Nette\Bridges\ApplicationLatte\ILatteFactory
{
	private $container;


	public function __construct(Container_a2d6a3effc $container)
	{
